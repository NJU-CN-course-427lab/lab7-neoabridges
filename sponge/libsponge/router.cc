...
void Router::route_one_datagram(InternetDatagram &dgram) {
    ...
    for (size_t i = 0; i < _route_table.size(); i++) {
        if (prefix_equal(dst_ip, _route_table[i].route_prefix, _route_table[i].prefix_length)) {
            if (!route_found || item.prefix_length < _route_table[i].prefix_length) {
                item = _route_table[i];
                route_found = true;
            }
        }
    }
    if (!route_found) return;
